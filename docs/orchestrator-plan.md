...
~/.claude/skills/guesung/        (전역 플러그인, ~/.claude git 레포로 버전 관리됨)
  범용 스킬 — spec-build · ui-verify · md · pr · convention-review · review-feedback-log · advance-code
        ▲ 이름으로 호출 (/guesung:xxx)
        │
...
단계별 대응은 [plugin/dependencies.md](../plugin/dependencies.md)가 SSOT다. `/guesung:full-review`는 후보였다가 뺐고(이유는 [OS.md](../OS.md) 4.9), 셀프 QA는 `/os:self-qa` + `/guesung:ui-verify` 조합이 맡는다.
...
│   ├── notion-databases.md         # 노션 DB 카탈로그. 식별자·속성·접근 규칙의 SSOT
│   ├── ledger/
...
★ 논의 필요 확인                        코드를 바꾸는 것만 묻는다. 고쳤으면 ②부터 한 회차 더
...
- 사람 개입은 **시작(설계서 확정)과 끝(최종 검토)** 이 뼈대이고, 그 사이에 루프가 먼저 말을 거는 자리가 **세 곳** 있다 — ① 설계서로 판단이 안 될 때, ⑦ 앞의 `논의 필요` 확인, 마무리의 일지 기록 승인(`/os:problem-log`). 셋 다 **미루는 비용이 묻는 비용보다 큰 곳**이라 남겼다. ②~⑤ 판정 구간에서는 묻지 않는다. (b)의 제안 선택은 ⑦ 앞 확인에 합친다.
...
- **검증**: 실제 대상 레포에서 작은 기능 1개를 end-to-end로 돌린다. → 이 시점에 §6의 미결 사항을 확정해야 한다.
  ...
  - ✅ 2026-08-31 — cashwalk 밖 얇은 경로(①→⑥→⑦→⑧) 제거, **시작 거부(하드 블록)** 로 교체. OS.md 6절 "이 OS는 회사 업무 전용" 확정에 따름. 아래 "프로젝트별 루프 범위" 절 개정
  - ⏳ end-to-end 미실행. 슬랙 스레드 1개로 work-card → design → implement-loop 순서로 돌려봐야 한다

### 프로젝트별 루프 범위 — cashwalk 밖은 시작 거부

> **개정 (2026-08-31).** 처음에는 cashwalk 밖에서 ②~⑤ 퀄리티 판정 구간만 건너뛰고 ①→⑥→⑦→⑧ 직선을 돌려주는 얇은 경로를 뒀다. 그러나 [OS.md](../OS.md) 6절에서 **"이 OS의 범위는 회사 업무로 한정한다"** 를 확정하면서, "범위 밖인데 절반은 돌려준다"는 어정쩡한 상태가 됐고 스킬마다 scope 분기가 자라는 비용만 남았다. 얇은 경로를 제거하고 **하드 블록**으로 바꿨다.

- **판정**은 `/os:work-card`의 프로젝트 판정 표와 같은 경로 규칙을 쓴다. 대상 레포 경로가 `레포지토리/cashwalk/` 아래면 진행, 그 외(`레포지토리/guesung/**` 등)는 `/os:design`·`/os:implement-loop` 모두 **시작을 거부**하고 한 줄로 안내한다(implement-loop 정지 조건 G).
- 경로를 정할 수 없으면 찍지 않고 사용자에게 대상 레포를 확인한다.
- **예외** — 로컬 설계서 파일 경로 입력(개발·검증용)은 막지 않는다. 루프 자체를 시험하는 경로다.

이 판정의 역할은 지원이 아니라 **가드레일**이다. `ledger.yaml`은 회사 코드리뷰에서 받은 지적으로 채워지는 팀 기준이라, 실수로 개인 레포에서 루프를 돌렸을 때 그 원장을 들이대면 맞지 않는 판정이 나온다. [OS.md](../OS.md) 1절의 "리뷰 지식을 먹고 자란다"는 축은 리뷰어가 있는 회사 업무에서만 돈다 — **cashwalk 밖 작업은 원장에 기여하지도, 원장을 쓰지도 않는다.** 개인 프로젝트는 이 OS의 범위 밖이고(OS.md 6절), 그쪽이 필요로 하는 앞단(기획·디자인의 발산)은 범용 스킬의 몫이다.
