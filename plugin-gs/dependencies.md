...
이 플러그인은 범용 스킬을 **이름으로 참조**한다. 전역 플러그인 `~/.claude/skills/guesung/`이 SSOT이며, 이 폴더로 복사·symlink 하지 않는다.

| 스킬 | 쓰이는 단계 | 없으면 |
|---|---|---|
| `/guesung:ui-verify` | ② 셀프 QA — `/gs:self-qa`가 자동 항목 판정에 호출 | 자동 항목을 전부 수동으로 돌린다 |
| `/guesung:pr` | ③ Draft PR | `gh pr create --draft`로 직접 만든다 |
